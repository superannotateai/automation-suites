...
from bsn_utils import (generate_bsp_feature,
...
def generate_proposals(ann_file, tem_results_dir, pgm_proposals_dir,
                       include_gt, pgm_proposals_thread, **kwargs):
    ...
    header = 'tmin,tmax,tmin_score,tmax_score,score'
    if include_gt:
        header += ',match_iou,match_ioa'
...
def parse_args():
    ...
    parser.add_argument(
        ...
        default='test',
        help='train or test')
    parser.add_argument('--include_gt', action='store_true', help='compute iou and ioa')
...
def main():
    ...
    if args.mode == 'test':
        generate_proposals(cfg.ann_file_val, tem_results_dir,
                           pgm_proposals_dir, args.include_gt, **cfg.pgm_proposals_cfg)
