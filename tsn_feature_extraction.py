...
def parse_args():
    ...
    parser.add_argument('--annot-json', default='sa_dataset/sa_dataset_activitynet.json', help='path to annotations json')
    parser.add_argument(
        '--data-list',
...
def main():
    ...
    with open(args.annot_json) as f:
    ...
    for item in data:
        ...
        annot_data['database'][osp.basename(frame_dir)]['feature_frame'] = feat.shape[0] * args.frame_interval
        with open(output_file, 'wb') as fout:
            pickle.dump(feat, fout)
        prog_bar.update()
    if args.modality == 'RGB':
        with open(args.annot_json, 'w') as f:
